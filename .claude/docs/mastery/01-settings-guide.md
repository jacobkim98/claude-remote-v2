...
## 코딩 컨벤션
[언어별 스타일 가이드, 금지 패턴]
- 상세 규칙 문서: {detected_conventions_path} (없으면 생략)
...
## 문서 참조
[프로젝트 docs/ 폴더 분석 후 실제 경로 기재]
- 도메인 명세: {detected_domains_path}
- API 문서: {detected_api_path}
- 개발 가이드: {detected_dev_path}
- docs/ 폴더가 없으면 이 섹션 생략

## Output Style Rules (응답 스타일 규칙)

질문 유형에 따라 응답 스타일을 자동으로 조절합니다.

### 코드 작성 요청
- 코드 먼저 제시, 설명은 간결하게 (1-2문장)

### "왜?" / 개념 질문
- 개념부터 설명, 단계별로 상세히, 예시 코드 포함

### 에러/버그 해결
- 원인 분석 → 해결책 → 재발 방지법

### 코드 리뷰 요청
- 문제점만 나열 (잘된 점 생략), 수정 코드 제시
- 심각도: ❌ Critical / ⚠️ Warning / 💡 Suggestion

### 리팩토링 요청
- Before/After 비교, 변경 이유 간단히

### 단순 질문 (Yes/No)
- 핵심만 답변, 불필요한 서론 생략

## Claude Code 설정 가이드
[Claude Code 학습 및 설정 생성 참조]
- **마스터 가이드**: `.claude/docs/CLAUDE-CODE-MASTERY.md`

### 슬래시 커맨드
- `/learn-claude-code` - Claude Code 사용법 학습
- `/setup-claude-code` - 프로젝트 설정 자동 생성
...
### 기본 MCP 서버 (모든 프로젝트 권장)

| 서버 | 용도 | 설치 명령어 |
|------|------|------------|
| **Slack** | 팀 커뮤니케이션, 알림 | `claude mcp add slack` |
| **GitHub** | PR/이슈 관리, 코드 리뷰 | `claude mcp add github` |
...
`/setup-claude-code` 실행 시 프로젝트 의존성을 분석하여 MCP 서버를 추천합니다:

```
1. 기본 서버 (자동 추가)
   └── Slack, GitHub

2. 의존성 분석 (package.json, requirements.txt 등)
